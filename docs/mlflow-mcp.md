# Using the official MLflow MCP alongside xprof
...
surface. xprof **reuses it** for run discovery rather than reinventing that surface; the analysis MCP
...
- `MLFLOW_MCP_TOOLS=ml` (or `all`) so the **experiments + runs** tools are exposed. The default
  `genai` exposes traces/scorers, which xprof does not use — set this or you get no run tools.
- MLflow MCP speaks **stdio**. To reach it from an HTTP-only client, put any stdio→streamable-http
  bridge (e.g. supergateway) in front of it; that bridging is a deployment detail, not part of xprof.
...
- `search_experiments` / `get_experiment`, `list_runs` / `describe_run` → a run's metrics, params,
  ...
  is MLflow MCP's job.
- The gap: `describe_run.artifact_uri` is the MLflow artifact-store URI, not xprof's `artifacts_uri`
  ...
  the xprof analysis MCP (`resolve_artifacts` / `analyze`).
...
| Resolve an id/alias to the profile file on disk, run nsys/neuron analysis | xprof `resolve_artifacts` / `analyze` |

Register both MCPs in your client: MLflow MCP to find and inspect the run, xprof to profile its
artifacts.
